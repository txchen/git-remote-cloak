...
Binary version `v0.3.0` writes Ciphertext Repository format `v1.0`. These versions are independent. The current operationally verified target is Linux amd64.
...
curl -fsSL https://github.com/txchen/git-remote-cloak/releases/latest/download/install.sh \
  | CLOAK_VERSION=v0.3.0 CLOAK_INSTALL_DIR="$HOME/.local/bin" bash
```
...
Hosted `cloak-storage` commits represent encrypted publications, not original
Git commits. Automatic [Compaction](docs/linux.md#maintenance) may replace their
visible history with one storage commit; recovery preserves the pushed original
