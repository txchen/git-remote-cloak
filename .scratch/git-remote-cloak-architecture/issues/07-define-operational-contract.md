...
Automatic compaction is synchronous work performed by the remote helper before a logical push, not a daemon. It triggers before creating a thirty-third live Pack Payload. The original byte trigger ran when ciphertext added since the previous Compaction reached 50% of the previous compacted snapshot size; later policy research found that this caused repeated full rebuilds for tiny repositories and isolated large additions. The current byte trigger also requires at least eight live Pack Payloads and 1 MiB of added ciphertext. A failed compaction leaves the old Storage Ref and the requested logical push unchanged. Progress reports packing, encryption, upload, validation, and publication separately.
